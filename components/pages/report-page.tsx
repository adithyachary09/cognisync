...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
             ...
             {/* Dynamic Gradient Pill for PDF */}
             <motion.button 
                whileHover={{ scale: 1.05, boxShadow: "0 20px 40px -15px rgba(249, 115, 22, 0.4)" }}
                whileTap={{ scale: 0.95 }}
                onClick={handlePrintPDF} 
                disabled={isExporting}
                className="flex items-center justify-center gap-2 bg-gradient-to-r from-orange-500 to-amber-600 text-white shadow-xl shadow-orange-500/20 rounded-full px-8 h-12 font-bold transition-all border border-white/10 relative overflow-hidden group"
             >
...
        {/* --- DYNAMIC TABS (Pill Style) --- */}
        <div className="mb-10 flex flex-col md:flex-row justify-between items-center gap-6">
            <div className="relative p-1.5 bg-slate-100 dark:bg-slate-900/60 backdrop-blur-xl rounded-full inline-flex shadow-inner border border-slate-200 dark:border-slate-800">
                {['today', 'history'].map((tab) => (
                    <button 
                        key={tab} 
                        onClick={() => setActiveTab(tab as any)} 
                        className={`relative px-8 py-3 rounded-full text-sm font-bold capitalize transition-all duration-500 z-10 flex items-center gap-2 ${activeTab === tab ? 'text-white' : 'text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-white'}`}
                    >
                        {activeTab === tab && (
                            <motion.div 
                                layoutId="activeTab" 
                                className="absolute inset-0 bg-gradient-to-r from-amber-500 to-orange-600 rounded-full shadow-lg shadow-orange-500/30" 
            ...
            {activeTab === 'history' && (
                <div className="flex gap-2 p-1 bg-slate-100 dark:bg-slate-900/60 rounded-full border border-slate-200 dark:border-slate-800">
                    {[7, 30, 90].map((d) => (
                        <motion.button
                            key={d}
